                        ...
                        uint8_t powe, uint32_t freq, uint8_t crc, uint8_t iiq, byte CE)
{
    ...
    if (ReadRegister(REG_OPMODE, CE) && OPMODE_LORA != 0 && debug>=2) {
        printf("Not 0\n");
    }
...
int sendPacket(uint8_t *buff_down, uint8_t length, byte CE) {
    ...
    if (debug>=2) {
        printf("Sending frequency from server package: %lf\n", ff);
    }
